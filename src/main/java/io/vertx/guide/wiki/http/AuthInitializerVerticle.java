...
public class AuthInitializerVerticle extends AbstractVerticle {
  ...
  public void start() throws Exception {
    ...
    List<String> dataInit = Arrays.asList(
        ...
        "insert into roles_perms values ('editor', 'update');", "insert into roles_perms values ('writer', 'update');",
        "insert into roles_perms values ('admin', 'create');", "insert into roles_perms values ('admin', 'delete');",
    ...
    SQLClientHelper.usingConnectionCompletable(dbClient, connection -> connection.rxBatch(schemaCreation)
        .flatMap(rs -> connection.rxQuery("select count(*) from user;")).flatMapCompletable(rs -> {
          if (rs.getResults().get(0).getInteger(0) == 0) {
            logger.info("Need to insert data");
            return connection.rxBatch(dataInit).ignoreElement();
          } else {
            logger.info("No need to insert data");
            return Completable.complete();
          }
        })).subscribe(() -> logger.info("Authentication database prepared"),
