...
public class WikiDatabaseVerticle extends AbstractVerticle {
    ...
    public void start(Promise<Void> promise) throws Exception {
        ...
        /*
         * Interestingly we break an important principle in Vert.x which is to avoid
         * blocking APIs, but since there are no asynchronous APIs for accessing
         * resources on the classpath our options are limited. We could use the Vert.x
         * executeBlocking method to offload the blocking I/O operations from the event
         * loop to a worker thread, but since the data is very small there is no obvious
         * benefit in doing so.
         */
        loadSqlQueries();
        ...
        dbClient.getConnection(ar -> {
            if (ar.failed()) {
                ...
                connection.execute(sqlQueries.get(SqlQuery.CREATE_PAGES_TABLE), create -> { // Here is an example of
                                                                                            // using SQL queries.
                    connection.close();
                    if (create.failed()) {
                        ...
                        vertx.eventBus().consumer(config().getString(CONFIG_WIKIDB_QUEUE, "wikidb.queue"),
                                this::onMessage); // The consumer method registers an event bus destination handler.
                        promise.complete();
                    }
                });
            }
